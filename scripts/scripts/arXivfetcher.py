...
def gpt_summarize(abstract):
    abstract = abstract.replace("\n", " ")
    prompt = "You are a expert in this section, You are working in academic position.Speak in Japanese, 日本語で話してください"
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "summarize the text"+abstract}
            ],
            max_tokens=3000)
        return response.choices[0].message.content
    except Exception as e:
        print(f"Failed to get GPT response: {str(e)}")
        return None

def print_markdown(arxiv_url, data, bibtex, gpt_abstarct, token, repo):
    markdown_content = (
        f"## 書誌情報\n"
        f"### タイトル\n{data['title']}\n \n"
        f"### URL\n{arxiv_url}\n \n"
...
def main():
    ...
    if response is not None:
        ...
        if data:
            ...
            summary= gpt_summarize(data['abstract'])
            print_markdown(arxiv_link, data, bibtex, summary, GITHUB_TOKEN, GITHUB_REPO)
        else:
